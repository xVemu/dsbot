...
const latestPatch = await fetchJson('https://ddragon.leagueoflegends.com/api/versions.json').then((res) => res[0])
...
export default {
  ...
  async execute(msg, [nameArg, regionArg]) {
    ...
    const { summonerLevel, profileIconId } = await fetchJson(
      ...
      const rankList = await fetchJson(`${baseURL}lol/league/v4/entries/by-puuid/${puuid}?api_key=${config.lolKey}`)

      const embedded = new EmbedBuilder({
        title: nameArg.value,
        color: 0x10B5BF,
        author: { name: 'League Of Legends Stats' },
        thumbnail: { url: `https://ddragon.leagueoflegends.com/cdn/${latestPatch}/img/profileicon/${profileIconId}.png` },
        ...
        fields: [
          {
            ...
            value: `${tier} ${rank} ${leaguePoints}LP - ${Math.round(wins/(losses+wins)*100)}% WR (${wins}W/${losses}L)`,
          })),
        ],
      })
